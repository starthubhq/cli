...
use crate::models::{ShManifest, ShKind, HubClient};
...
impl ExecutionEngine {
    ...
    async fn run_action_tree(&self, action_state: &mut ActionState, inputs: &HashMap<String, Value>) -> Result<()> {
        // 1. Parse and validate inputs according to manifest structure
        self.parse_and_validate_inputs(action_state, inputs)?;
        
        Ok(())
    }

    fn parse_and_validate_inputs(&self, action_state: &mut ActionState, inputs: &HashMap<String, Value>) -> Result<()> {
        println!("Parsing and validating inputs");
        println!("Inputs: {:?}", inputs);
        // Validate each input against its specified type
        for (input_key, input_value) in inputs {
            // Extract the type from the input (assuming it has a "type" field)
            let input_type = if let Some(type_value) = input_value.get("type") {
                if let Some(type_str) = type_value.as_str() {
                    type_str
                } else {
                    return Err(anyhow::anyhow!("Input '{}' has invalid type field - must be string", input_key));
                }
            } else {
                return Err(anyhow::anyhow!("Input '{}' missing required 'type' field", input_key));
            };

            println!("Input type: {}", input_type);
            
            // Look up the specific type definition and convert to JSON Schema
            if let Some(types) = &action_state.types {
                if let Some(type_def) = types.get(input_type) {
                    // Convert manifest type format to JSON Schema format
                    let json_schema = self.convert_manifest_type_to_json_schema(type_def)?;
                    let compiled_schema = JSONSchema::compile(&json_schema)
                        .map_err(|e| anyhow::anyhow!("Failed to compile JSON schema: {}", e))?;
                    
                    // Validate input against this specific type's schema
                    let validation_result = compiled_schema.validate(input_value);
                    if let Err(validation_errors) = validation_result {
                        let error_messages: Vec<String> = validation_errors
                            .map(|error| format!("Input '{}' validation error: {}", input_key, error))
                            .collect();
                        return Err(anyhow::anyhow!("Validation failed for input '{}': {}", input_key, error_messages.join(", ")));
                    }
                } else {
                    return Err(anyhow::anyhow!("Input '{}' references unknown type '{}'", input_key, input_type));
                }
            } else {
                return Err(anyhow::anyhow!("No types defined in manifest"));
            }
            
            // Parse inputs into structured format
            let mut structured_input = serde_json::Map::new();
            structured_input.insert("name".to_string(), Value::String(input_key.clone()));
            structured_input.insert("type".to_string(), Value::String(input_type.to_string()));
            structured_input.insert("value".to_string(), input_value.clone());
            
            action_state.inputs.push(Value::Object(structured_input));
        }
        
        Ok(())
    }

    fn convert_manifest_type_to_json_schema(&self, type_def: &Value) -> Result<Value> {
        match type_def {
            Value::Object(fields) => {
                ...
                for (field_name, field_def) in fields {
                    if let Some(field_obj) = field_def.as_object() {
                        ...
                        let field_required = field_obj.get("required")
                            .and_then(|r| r.as_bool())
                            .unwrap_or(false);
                        
                        let field_description = field_obj.get("description")
                            .and_then(|d| d.as_str());
                        
                        // Create property schema
                        let mut property_schema = serde_json::Map::new();
                        property_schema.insert("type".to_string(), Value::String(field_type.to_string()));
                        
                        if let Some(desc) = field_description {
                            property_schema.insert("description".to_string(), Value::String(desc.to_string()));
                        }
                        
                        properties.insert(field_name.clone(), Value::Object(property_schema));
                ...
                if !required_fields.is_empty() {
                    schema.insert("required".to_string(), Value::Array(
                        required_fields.into_iter().map(Value::String).collect()
                    ));
                }
                
                Ok(Value::Object(schema))
            },
            _ => Err(anyhow::anyhow!("Invalid type definition format"))
        }
    }
    ...
    async fn build_action_tree(&self,
        action_ref: &str,
        parent_action_id: Option<&str>) -> Result<ActionState> {
    ...
    async fn topological_sort_composition_steps(&self, children: &HashMap<String, ActionState>) -> Result<Vec<String>> {
    ...
    //     // Create a parent ID for the action. This is going to be useful to navigate the action tree.
    ...
    //             // TODO: maybe we can improve this by injecting the global inputs later on, so we
    //             // split between tree creation and variable resolution.
    ...
    //     // We already have enough information from the current manifest to understand how
    //     // its children depend on each other.
    //     let sorted_steps = self.topological_sort_composition_steps(&manifest).await?;
        
    //     // DFS to build the action tree. We will recursively download manifests for its children 
    //     // as deep as possible first, and then go to the next child.
    //     // 4. First pass: Build all child actions without setting execution order
    //     // Child actions are defined as a key value pair, since the functionality of
    //     // each step is unique, even if maybe the underlying action being used is the same.
    ...
    //                 // 7. Create child composition action - this will recursively download manifests for its children
    ...
    //                     inputs: {
    //                         let step_map = step_definition.as_object().ok_or_else(|| anyhow::anyhow!("Step definition is not an object"))?;
    //                         self.build_inputs_from_step_definition(step_map, &child_manifest)?
    //                     },
    //                     outputs: Vec::new(),
    ...
    //                         // Manifest structure fields
    //                         types: if child_manifest.types.is_empty() { None } else { Some(child_manifest.types.clone().into_iter().collect()) },
    //                 };
