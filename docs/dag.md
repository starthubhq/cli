# Dependency Tree Resolution and DAG Construction

## Overview

The dependency tree resolution process transforms a StartHub composition manifest into an executable DAG (Directed Acyclic Graph) by recursively expanding composition steps into atomic WASM/Docker steps while preserving variable dependencies.

## Process Flow

### 1. Initial State
Given a `starthub-lock.json` manifest, we start with:
```json
{
  "inputs": {
    "weather_config": {
...
For each step in the composition:

#### A. If Step is WASM/Docker (Atomic)
- **Download artifacts** for the target action
- **Create StepState** with resolved inputs
- **No further recursion needed**

#### B. If Step is Composition
- **Recursively traverse** the composition's steps
- **Before recursion**: Map variable dependencies
- **After recursion**: Merge expanded steps into execution plan

### 3. Variable Dependency Mapping

Before going one level deeper in recursion, we need to:

1. **Extract template variables** from the composition step's inputs
2. **Map variable sources** to target steps
3. **Build dependency graph** for variable resolution

#### Example Mapping:
```rust
// For step "get_coordinates" with inputs:
{
  "open_weather_config": {
...
VariableMapping {
  ...
  source_step: "inputs",
  source_path: "weather_config.location_name",
  target_path: "open_weather_config.location_name"
}
```

### 4. Final DAG State

The end result is an execution state with:
...
ExecutionState {
  ...
  steps: [
    StepState {
      id: "d1acfe83-51c5-4aa5-aa5f-94dc7a56954d",
      original_name: "get_geocoding_response",  // From composition step
    ...
    StepState {
      id: "90223ff0-7a90-47c0-9285-d3714c43f1d3", 
      original_name: "get_weather_response",  // From composition step
      uses: "starthubhq/http-get-wasm:0.0.16",
      kind: "wasm",
      inputs: {
        "url": "https://api.openweathermap.org/data/2.5/weather?lat={{inputs.weather_config.lat}}&lon={{inputs.weather_config.lon}}&appid=c646e7e39a20317bd056f6ff501d2ab2"
      },
      outputs: {}  // Empty initially
    }
  ],
  data_flow: [
    // Maps variable dependencies between steps
    DataFlowEdge {
      from_step: "inputs",
...
- **Template variables** like `{{inputs.weather_config.location_name}}` need resolution
- **Source mapping** tracks where each variable comes from
- **Target mapping** tracks where each variable is used

### Step Name Preservation
- **original_name** preserves the composition step name for variable resolution
- **id** is a generated UUID for internal tracking
- **uses** is the target action reference

### Data Flow Tracking
- **Global data flow graph** shows all variable dependencies
- **Step-to-step dependencies** for proper execution order
- **Input-to-step dependencies** for initial variable resolution

## Implementation Notes

### Before Recursion (Composition Steps)
1. Extract template variables from step inputs
2. Map variable sources to target steps  
3. Build dependency edges
4. Preserve composition step context

### During Recursion (Atomic Steps)
1. Download artifacts for WASM/Docker actions
2. Create StepState with resolved inputs
3. Maintain original step names for variable resolution

### After Recursion (DAG Construction)
1. Merge all expanded steps into execution plan
2. Build global data flow graph
3. Validate dependency resolution
4. Prepare for execution

## Benefits

1. **Clear separation** between composition logic and execution logic
2. **Preserved variable dependencies** through recursion levels
3. **Efficient execution** with pre-resolved variable mappings
4. **Debugging support** with complete data flow visibility
5. **Scalable architecture** for complex nested compositions
